...
## 이식에 남은 결함 둘 — 위의 무효 블록과 별개다

둘 다 실측됐고 아직 안 고쳤다. 히어로 박스는 하네스 README §5.6 각주 (2)에 `LCP size`와
함께 적혀 있고, **`constrained` 건은 아래 실측값이 전부다** — README에도 없다.

- **히어로 이미지 박스가 깨져 있다.** 코어 `.wp-block-image.alignfull img { height: auto }`
  (0,2,1)가 `.hero__media { height: 120% }`(0,1,0)를 이긴다. 1,080이어야 할 박스가 975.94,
  하단에 14px 틈, `object-fit`이 안 자른다 — **아트디렉션이 Phase 1과 갈렸다.**
- **`layout: constrained`가 자식을 `contentSize`로 가둔다.** `.journal__item`은 호스트가
  572인데 자식 셋이 전부 544다. `.card`는 368이라 지금은 숨는데, **편집자가 카드를 하나
  지우면 572가 되어 즉시 드러난다.** 그 조작은 PR-6이 판정 기준으로 열어 둔 것이다.
