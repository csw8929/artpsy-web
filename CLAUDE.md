...
## 에디터에서 히어로·저널 이미지 블록을 클릭하지 않는다

`d4b217a`에서 `core/image` 블록 **셋이 무효**다(`hero__media-block`, `journal__thumb-block`
×2 — `figure`에 `has-warning`이 붙는다). 경고 UI는 **그 이미지를 클릭해야** 뜨고, 뜨면
에디터가 **블록 복구**를 권한다. 복구를 누르면 마크업이 `save` 출력으로 다시 써지면서
**`class="hero__media"`가 사라진다** — 그 클래스가 히어로 CSS 전부의 걸이다.
...
## 이식에 남은 결함 하나 — 위의 무효 블록과 별개다

**히어로 이미지 박스는 Phase 3에서 닫혔다** (`#27`이 `core/image` 셋을 유효하게 만들면서
걸이를 구조 선택자로 옮겼고, 그 특정도가 코어를 이겨 박스가 975.94에서 1080이 됐다).
하네스 README §5.6 각주 (2)는 그 뒤 상태로 갱신돼 있다.
...
다음 씨앗은 manager가 쥐고 있고 1번이 히어로 LCP다. 그 첫 홉은
